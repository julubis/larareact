        ...
        <AuthLayout user={auth.user}>
            ...
            <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 w-full gap-4 bg-white p-4 rounded-md">
                <div className="max-w-screen-sm">
                    ...
                    {isEdit ? <input value={data.name} onChange={(e) => setData('name', e.target.value)} type="text" name="phone" className="w-full rounded-md" autoFocus /> : <div className="w-full rounded-md text-sm border border-gray-300 p-2">{distributor.name}</div>}
                ...
                <div className="max-w-screen-sm">
                    ...
                    {isEdit ? <input value={data.phone} onChange={(e) => setData('phone', e.target.value)} type="text" name="phone" className="w-full rounded-md" /> : <div className="w-full rounded-md text-sm border border-gray-300 p-2">{distributor.phone}</div>}
                </div>
                <div className="max-w-screen-sm sm:col-span-2">
                    ...
                    {isEdit ? <textarea value={data.address} onChange={(e) => setData('address', e.target.value)} name="address" className="w-full rounded-md bg-gray-50 border border-gray-300" ></textarea> : <div className="w-full rounded-md text-sm border border-gray-300 p-2">{distributor.address}</div>}
                    
                </div>
                <div className="max-w-screen-sm sm:col-span-3 flex gap-2">
                    {!isEdit && (
                        <>
                        <button onClick={() => setIsEdit(true)} className="flex gap-2 mb-2 text-black border border-gray-300 bg-white hover:text-white hover:bg-primary-500 focus:ring focus:outline-none focus:border-none focus:ring-primary-300 font-medium rounded-md text-sm px-5 py-2.5 text-center"><Pencil className="w-5 h-5"/>Edit</button>
                        <Link href={`/distributors/D${distributor.id.toString().padStart(3, '0')}`} as="button" method="delete" className="flex gap-2 mb-2 text-black border border-gray-300 bg-white hover:text-white hover:bg-primary-500 focus:ring focus:outline-none focus:border-none focus:ring-primary-300 font-medium rounded-md text-sm px-5 py-2.5 text-center"><Trash className="h-5 w-5"/>Hapus</Link>
                        </>
                        )
                    }
                    {isEdit && (
